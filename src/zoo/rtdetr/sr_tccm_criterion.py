...
# -*- coding: utf-8 -*-
"""
SR-TCCM for RT-DETRv2
=====================
Scale-Reliability and Quality-aware Trajectory-Consistent Competition Margin.
...
@register()
class SRTCCMCriterionv2(RTDETRCriterionv2):
    ...
    def __init__(
        ...
        sr_use_quality_gate: bool = False,
        ...
        self.sr_use_quality_gate = bool(sr_use_quality_gate)
    ...
    def _scale_reliability(self, target_box: torch.Tensor) -> torch.Tensor:
        if not self.sr_use_scale_reliability:
            return target_box.new_tensor(1.0)
        area = (target_box[2] * target_box[3]).detach().clamp_min(self.eps)
        relative = torch.sqrt(area / self.sr_area_reference).clamp(max=1.0)
        return self.sr_scale_floor + (1.0 - self.sr_scale_floor) * relative
    ...
    def _raw_rank_loss(
        ...
        for batch_index, target in enumerate(targets):
            ...
            for winner_q_raw, target_j_raw in zip(src_indices.tolist(), tgt_indices.tolist()):
                ...
                if self.sr_use_quality_gate:
                    valid_quality = quality_delta.gt(0.0)
                    candidate_indices = candidate_indices[valid_quality]
                    persistence = persistence[valid_quality]
                    candidate_iou = candidate_iou[valid_quality]
                    candidate_quality = candidate_quality[valid_quality]
                    quality_delta = quality_delta[valid_quality]
                    if candidate_indices.numel() == 0:
                        continue
                    quality_gate = torch.sigmoid(quality_delta / self.sr_quality_tau)
                else:
                    quality_gate = torch.ones_like(persistence)

                keep = min(self.sr_max_competitors, int(candidate_indices.numel()))
                top_values, top_pos = torch.topk(
                    persistence * quality_gate,
                    k=keep,
                    largest=True,
                    sorted=True,
                )
                ...
                scale_gate = self._scale_reliability(gt_boxes[target_j])
                winner_gate = torch.sigmoid(
                    (winner_iou - competitor_iou) / self.sr_winner_iou_tau
                )
                pair_weight = (
                    top_values * scale_gate * winner_gate * competitor_quality_gate
                ...
                margin = (
                    self.sr_margin_base + self.sr_margin_scale * top_values.detach()
                )
